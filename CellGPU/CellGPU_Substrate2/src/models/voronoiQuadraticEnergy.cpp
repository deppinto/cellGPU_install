...
VoronoiQuadraticEnergy::VoronoiQuadraticEnergy(int n, bool reprod, Dscalar A0, Dscalar P0, Dscalar V0, int dx, Dscalar IncrementationValue, int SubIntType)
    {
    printf("Initializing %i cells with random positions in a square box on substrate... \n",n);
    Reproducible = reprod;
    initializeVoronoiQuadraticEnergy(n);
    //setSubstratePreferences(SubIntType, IncrementationValue, dx);

    switch(SubIntType)
    {
    case 1:
        setCellPreferencesSubstrate(A0, P0, true, dx);
        setv0DrSubstrate(V0, 1.0, dx);
    break;
    case 2:
        setCellPreferencesSubstrate(A0, P0, false, dx);
        setv0DrSubstrate(V0, 1.0, dx);
    break;
    case 3:
        setCellPreferencesSubstrate(A0, P0, true, dx);
