...
void Simple2DCell::initializeSimple2DCell(int n)
    ...
    //setting cell positions randomly also auto-generates a square box with L = 100
...
*/
void Simple2DCell::setCellRadiusUniform(Dscalar l)
    {
    cellRadius.resize(Ncells);
    ArrayHandle<Dscalar> h_l(cellRadius,access_location::host,access_mode::overwrite);

    if(l>0)NonConfluent=true;
...
*/
void Simple2DCell::setCellRadius(vector<Dscalar> &Radius)
    {
    cellRadius.resize(Ncells);
    ArrayHandle<Dscalar> h_l(cellRadius,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
	  if(Radius[ii]>0)NonConfluent=true;
	  h_l.data[ii] = Radius[ii];
        };
    };
...
void Simple2DCell::computeGeometry()
    ...
    if(GPUcompute)
      ...
      {
        if(NonConfluent==true)computeNonConfluentGeometryCPU();
        else computeGeometryCPU();
      }
    }
...
void Simple2DCell::setCellPositionsRandomly()
    ...
    Dscalar addval=0;
    Dscalar IniSize=sqrt((Dscalar)Ncells/2);

    addval=boxsize-IniSize;
    addval/=2;
...
void Simple2DCell::setCellAdaptationTimeUniform(Dscalar tau)
    {
    AdaptationTime.resize(Ncells);
    ArrayHandle<Dscalar> h_tau(AdaptationTime,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        h_tau.data[ii] = tau;
        };
 ...
 * */
void Simple2DCell::setCellAdaptationTime(vector<Dscalar> &tau)
    {
    AdaptationTime.resize(Ncells);
    ArrayHandle<Dscalar> h_tau(AdaptationTime,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
