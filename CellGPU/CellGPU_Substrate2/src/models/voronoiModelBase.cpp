...
void voronoiModelBase::initializeVoronoiModelBase(int n)
    ...
    particleExclusionsRadius=false;
...
/*!
*/
void voronoiModelBase::setExclusionsRadius(int cn)
    {
    particleExclusions=true;
    particleExclusionsRadius=true;
    external_forces.resize(Ncells);
    exclusions.resize(Ncells);
    ArrayHandle<Dscalar2> h_mot(Motility,access_location::host,access_mode::readwrite);
    ...
    Dscalar addval=0;
    Dscalar IniSize=sqrt((Dscalar)cn/2);

    addval=x11-IniSize;
    addval/=2;
    Dscalar theta=2*PI/(Ncells-cn);
    Dscalar Rad=x11/2-0.1;
        ...
        if( ii >= cn )
            {
                ...
                Dscalar x = x11/2+Rad*cos(theta*(ii-cn));
                Dscalar y = x11/2+Rad*sin(theta*(ii-cn));
                h_p.data[ii].x = x;
                h_p.data[ii].y = y;
            }
	else
	{
	        Dscalar x = noise.getRealUniform(0.0,IniSize)+addval;
	        Dscalar y = noise.getRealUniform(0.0,IniSize)+addval;
        	h_p.data[ii].x = x;
	        h_p.data[ii].y = y;
		/*Dscalar x, y;
		if(ii==0)
        ...
        globalTriangulationCGAL();
        resetLists();
	computeNonConfluentGeometryCPU();
    };

//Set substrate preferences
void voronoiModelBase::setSubstratePreferences(int SubIntType, Dscalar IncrementationValue, int dx, double sigmac)
    {
    SubstrateInteractions.resize(4);
    SubstrateGrid.resize(dx*dx);
    ArrayHandle<Dscalar> h_si(SubstrateInteractions,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar> h_sg(SubstrateGrid,access_location::host,access_mode::overwrite);
...
void voronoiModelBase::setSubstratePreferencesRnd(int SubIntType, Dscalar IncrementationValue, int dx, double arguments[], double sigmac)
    {
    SubstrateInteractions.resize(4);
    SubstrateGrid.resize(dx*dx);
    ArrayHandle<Dscalar> h_si(SubstrateInteractions,access_location::host,access_mode::overwrite);
    ...
    for(int ii=0; ii<dx*dx; ii++)
        {
        h_sg.data[ii]=IncrementationValue*arguments[ii];
        }
    };
...
void voronoiModelBase::setCellPreferencesSubstrate(Dscalar A0, Dscalar P0, bool ChangeArea, int dx)
    {
    AreaPeriPreferences.resize(Ncells);
    CellTime.resize(Ncells);
    ArrayHandle<Dscalar2> h_ap(AreaPeriPreferences,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::read);
    ArrayHandle<Dscalar> h_sg(SubstrateGrid,access_location::host,access_mode::read);
    ArrayHandle<Dscalar> h_ct(CellTime,access_location::host,access_mode::overwrite);
...
*/
void voronoiModelBase::setv0DrSubstrate(Dscalar v0new,Dscalar drnew, int dx)
    {
    ...
    for (int ii = 0; ii < Ncells; ++ii)
        ...
        int y=h_p.data[ii].y/deltax;
        int site = x+y*dx;
        h_mot.data[ii].x = v0new+h_sg.data[site];
        h_mot.data[ii].y = drnew;
        Dscalar theta = h_cd.data[ii];
        h_v.data[ii].x = h_mot.data[ii].x*cos(theta);
        h_v.data[ii].y = h_mot.data[ii].x*sin(theta);
        h_ct.data[ii]=h_mot.data[ii].x;
        }
    };
...
void voronoiModelBase::movePointsCPU(GPUArray<Dscalar2> &displacements,Dscalar scale)
    ...
    ArrayHandle<int> h_ex(exclusions,access_location::host,access_mode::read);
    ...
    if(scale == 1.)
	    ...
	    if(particleExclusionsRadius)
	    {
		    ...
		    {
	                    h_p.data[idx].x += CM.x/avg;
                            h_p.data[idx].y += CM.y/avg;
		    }
...
void voronoiModelBase::movePointsSubstrateCPU(GPUArray<Dscalar2> &displacements,Dscalar scale)
    {
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::readwrite);
    ArrayHandle<Dscalar2> h_d(displacements,access_location::host,access_mode::read);
    ArrayHandle<Dscalar2> h_ap(AreaPeriPreferences,access_location::host,access_mode::overwrite);
    ...
    //ArrayHandle<Dscalar2> h_v(cellVelocities);
    //ArrayHandle<Dscalar2> h_mot(Motility,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar> h_si(SubstrateInteractions,access_location::host,access_mode::read);
    ArrayHandle<Dscalar> h_sg(SubstrateGrid,access_location::host,access_mode::read);
    ArrayHandle<Dscalar> h_ct(CellTime,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar> h_tau(AdaptationTime,access_location::host,access_mode::read);

    int subinter=h_si.data[0];
    Dscalar x11,x12,x21,x22;
    Box->getBoxDims(x11,x12,x21,x22);
    Dscalar IncVal=0;
    Dscalar dx=h_si.data[2];
    Dscalar deltax=x11/dx;
    Dscalar p0;
    Dscalar omega=h_si.data[3];
...
void voronoiModelBase::globalTriangulationCGAL(bool verbose)
    ...
    completeRetriangulationPerformed = 1;
    DelaunayCGAL dcgal;
    //cout<<"GLOBAL CGAL------------------------------"<<timestep<<endl;
    ArrayHandle<Dscalar2> h_points(cellPositions,access_location::host, access_mode::read);
    vector<pair<Point,int> > Psnew(Ncells);
    for (int ii = 0; ii < Ncells; ++ii)
        {
	if(h_points.data[ii].x>boxsize || h_points.data[ii].x<0)cout<<"CGAL"<<endl;
        if(h_points.data[ii].y>boxsize || h_points.data[ii].y<0)cout<<"CGAL"<<endl;
...
void voronoiModelBase::writeTriangulation(ofstream &outfile)
    {
      ...
      for(int i=0; i<Ncells; i++)
        {
          int vnum=h_vn.data[i];
	  if(vnum>0)outfile << setprecision(15) << "VoroC(" <<  h_p.data[i].x << "," << h_p.data[i].y << "," << h_l.data[i]  << "," << color1 << "," << color2<< "," << color3  << ")"  << endl;
	  else outfile << setprecision(15) << "VoroA(" <<  h_p.data[i].x << "," << h_p.data[i].y << "," << h_l.data[i]  << "," << color1 << "," << color2<< "," << color3  << ")"  << endl;
          for(int j=0; j<vnum; j++)
            {
              Dscalar2 voro=h_v.data[nc_idx(j,i)];
              Dscalar2 voron=h_v.data[nc_idx((j+1)%vnum,i)];
              int typev=h_ht.data[nc_idx(j,i)];
              int typevn=h_ht.data[nc_idx((j+1)%vnum,i)];
              bool curved=true;

              if(typev==1 || typevn==1)curved=false;
              else if(typev==3 && typevn==4)curved=false;

              if(curved==false)outfile << setprecision(15) << "Voro(" << h_p.data[i].x+voro.x << "," << h_p.data[i].y+voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+voron.x << "," << h_p.data[i].y+voron.y << "," << color1 << "," << color2<< "," << color3  << ")"  << endl;
              else
              {
                      if(h_teta.data[nc_idx(j, i)]<PI)outfile << setprecision(15) << "VoroL(" << h_p.data[i].x+mult*voro.x << "," << h_p.data[i].y+mult*voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+mult*voron.x << "," << h_p.data[i].y+mult*voron.y << "," << color1 << "," << color2 << "," << color3  <<"," <<  h_l.data[i]  << ")"  << endl;
                      else outfile << setprecision(15) << "VoroD(" << h_p.data[i].x+mult*voro.x << "," << h_p.data[i].y+mult*voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+mult*voron.x << "," << h_p.data[i].y+mult*voron.y << "," << color1 << "," << color2 << "," << color3  <<"," <<  h_l.data[i]  << ")"  << endl;
              }
...
//similarly, write a text file with particle positions. This is often called when an exception is thrown
void voronoiModelBase::writeTriangulation(ofstream &outfile, Dscalar delta, Dscalar min, vector<Dscalar> Color, bool Tri)
    {
      //this outputs a povray file
      if(Tri)
      {
      ...
      Dscalar mult=100;//fuck this....try to fix later....
	  ...
	  for(int j=0; j<vnum; j++)
	    {
	      ...
	      if(curved==false)outfile << setprecision(15) << "Voro(" << h_p.data[i].x+voro.x << "," << h_p.data[i].y+voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+voron.x << "," << h_p.data[i].y+voron.y << "," << ((max-Color[i])/delta) << "," << ((Color[i]-min)/delta)*0.5 << "," << 0.01  << ")"  << endl;
	      else
	      { 
		      if(h_teta.data[nc_idx(j, i)]<PI)outfile << setprecision(15) << "VoroL(" << h_p.data[i].x+mult*voro.x << "," << h_p.data[i].y+mult*voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+mult*voron.x << "," << h_p.data[i].y+mult*voron.y << "," << ((max-Color[i])/delta) << "," << ((Color[i]-min)/delta)*0.5 << "," << 0.01  <<"," <<  h_l.data[i]  << ")"  << endl;
		      else outfile << setprecision(15) << "VoroD(" << h_p.data[i].x+mult*voro.x << "," << h_p.data[i].y+mult*voro.y << "," << h_p.data[i].x << "," << h_p.data[i].y << "," << h_p.data[i].x+mult*voron.x << "," << h_p.data[i].y+mult*voron.y << "," << ((max-Color[i])/delta) << "," << ((Color[i]-min)/delta)*0.5 << "," << 0.01  <<"," <<  h_l.data[i]  << ")"  << endl;
	      }
...
void voronoiModelBase::computeNonConfluentGeometryCPU()
{
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::read);
    ArrayHandle<Dscalar2> h_AP(AreaPeri,access_location::host,access_mode::readwrite);
    ArrayHandle<int> h_nn(cellNeighborNum,access_location::host,access_mode::read);
    ...
    ArrayHandle<int> h_ht(voroType,access_location::host,access_mode::overwrite);
    ArrayHandle<int> h_vn(voroNum,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar2> h_v(NCvoroCur,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar4> h_t(voroTriangulation,access_location::host,access_mode::overwrite);
    ArrayHandle<int> h_vcn(voroCellNeigh,access_location::host,access_mode::overwrite);
    ArrayHandle<Dscalar> h_teta(voroOutAngle,access_location::host,access_mode::overwrite);

    //int qq=74;

    //cout<<"Compute non congluent geometry----------------------------"<<endl;
    ...
    for (int i = 0; i < Ncells; ++i)
	  ...
	  {
	    ns[nn]=h_n.data[n_idx(nn,i)];
	    //if(i==qq)cout<<"NEIGH: "<<ns[nn]<<endl;
	  };
	
        //compute base set of voronoi points, and the derivatives of those points w/r/t cell i's position
	Dscalar l=h_l.data[i];
	Dscalar2 rij, rik, rig;
	Dscalar2 circumcent, circumcent2;
        Dscalar2 nnextp,nlastp, nafterp;
        Dscalar2 pi = h_p.data[i];
	int noNeigh=0;
	int ii=0;
	...
	nnextp = h_p.data[ns[0]];
	Box->minDist(nnextp,pi,rik);
	Circumcenter(rij,rik,circumcent);
	int id;
        //Dscalar2 rjc, rkc1, rkc2, rgc;
        //Box->minDist(rij,circumcent,rjc);
	//Box->minDist(rik,circumcent,rkc1);

        for (int nn = 0; nn < neigh;++nn)
	  {
	    nafterp = h_p.data[ns[(nn+1)%neigh]];
	    Box->minDist(nafterp,pi,rig);
	    Circumcenter(rik,rig,circumcent2);
	    id = nc_idx(ii,i);
            //Box->minDist(rik,circumcent2,rkc2);
	    //Box->minDist(rig,circumcent2,rgc);

	    //if(i==qq)cout<<"HERE1: "<<norm(circumcent)<<" "<<norm(circumcent2)<<" "<<norm(rij)<<" "<<norm(rik)<<" "<<norm(rig)<<endl;

	    if(norm(circumcent)<l)
	      {
		h_ht.data[id]=1;
		h_v.data[id] = circumcent;
		h_t.data[id].x=rij.x;
		h_t.data[id].y=rij.y;
		h_t.data[id].z=rik.x;
		h_t.data[id].w=rik.y;
		...
		//if(i==qq)cout<<"geometry: "<<circumcent.x<<" "<<circumcent.y<<" "<<ns[nn]<<endl;
		
		if(norm(circumcent2)>l)
		  {
		    ii++;
		    id = nc_idx(ii,i);
		    h_t.data[id].x=rik.x;
		    ...
		    Dscalar2 rki=(-1)*rik;
		    Dscalar2 rikT;
		    Transpose((-1)*rik, rikT);
		    Dscalar2 h1, h2;
		    h1=rik/2+rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    h2=rik/2-rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    if(dot(h1,h1)>1+0.00001 || dot(h2,h2)>1+0.00001){cout<<"ERROR3: "<<dot(h1,h1)<<" "<<dot(h2,h2)<<endl;exit(911);}

		    Dscalar teta0=h1.x*h2.y-h1.y*h2.x;
		    if(teta0>0)
		      {
			h_v.data[id] = h2;
			h_t.data[id].z=-1;
		...
		if(norm(circumcent2)<l)
		  {
		    ...
		    h1=rik/2+rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    h2=rik/2-rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    if(dot(h1,h1)>1+0.00001 || dot(h2,h2)>1+0.00001){cout<<"ERROR2: "<<dot(h1,h1)<<" "<<dot(h2,h2)<<endl;exit(911);}
		...
		else if(norm(rik)<2*l)
		  {
		    //check if the other cell is blocking the connection
		    bool connect=true;
		    ...
		    h1=rik/2+rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    h2=rik/2-rikT*sqrt(4*l*l-dot(rik,rik))/(2*sqrt(dot(rik,rik)));
		    if(dot(h1,h1)>1+0.00001 || dot(h2,h2)>1+0.00001){cout<<"ERROR1: "<<dot(h1,h1)<<" "<<dot(h2,h2)<<endl;exit(911);}

		    Dscalar2 rjh1, rjh2;
		    Dscalar2 rgh1, rgh2;
		    Box->minDist(rij,h1,rjh1);
		    Box->minDist(rij,h2,rjh2);
		    Box->minDist(rig,h1,rgh1);
		    Box->minDist(rig,h2,rgh2);

		    if(norm(rjh1)<l || norm(rjh2)<l || norm(rgh1)<l || norm(rgh2)<l)connect=false;

		    //if(i==qq)cout<<"CHECK: "<<norm(rjh1)<<" "<<norm(rjh2)<<" "<<norm(rgh1)<<" "<<norm(rgh2)<<endl;
			...
			//if(i==qq)cout<<"v: "<<h1.x<<" "<<h1.y<<" "<<h2.x<<" "<<h2.y<<endl;
			Dscalar teta0=h1.x*h2.y-h1.y*h2.x;
			if(teta0>0)
			  {
			    h_v.data[id] = h1;
		...
		else
		  {
            ...
            rij=rik;
	    rik=rig;
	    circumcent=circumcent2;
	    ii++;
	  };
	h_vn.data[i]=ii;

        vector<Dscalar2> voro(ii);
	vector<int> h_type(ii);
	for(int nn=0; nn<ii; nn++)
	  {
	    voro[nn]=h_v.data[nc_idx(nn, i)];
	    h_type[nn]=h_ht.data[nc_idx(nn, i)];
	    //if(i==qq)cout<<"VORO: "<<voro[nn].x<<" "<<voro[nn].y<<" "<<h_type[nn]<<endl;
	  };
		...
		switch(h_type[prev_nn])
		  ...
		  case 2:
			...
			Vperi += sqrt(dx*dx+dy*dy);
		      ...
		      {
			Dscalar deth=Det2x2(vlast.x, vlast.y, vnext.x, vnext.y);
			Dscalar doth=dot(vlast, vnext);
			Dscalar teta0=atan2(deth, doth);
			Dscalar teta;
			if(teta0>0)teta=teta0;
			else teta=2*PI+teta0;
			Varea += 0.5*l*l*teta;
			Vperi += l*teta;
			h_teta.data[nc_idx(prev_nn, i)]=teta;
		      }		    
		    break;
		  case 3:
		    Varea += TriangleArea(vlast,vnext);
...
//cout<<"COMPUTED NC GEOMETRY"<<endl;
};
...
void voronoiModelBase::resetLists()
    {
        voroType.resize(2*neighMax*Ncells);
        NCvoroCur.resize(2*neighMax*Ncells);
        voroTriangulation.resize(2*neighMax*Ncells);
        voroCellNeigh.resize(2*neighMax*Ncells);
        voroOutAngle.resize(2*neighMax*Ncells);
