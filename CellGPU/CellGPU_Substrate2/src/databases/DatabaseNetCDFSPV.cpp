...
void SPVDatabaseNetCDF::SetDimVar()
    ...
    AdaptationTimeVar    = File.add_var("AdaptationTime", ncDscalar,recDim, NvDim);
    if(exclusions)
...
void SPVDatabaseNetCDF::GetDimVar()
    ...
    AdaptationTimeVar = File.get_var("AdaptationTime");
    if(exclusions)
...
void SPVDatabaseNetCDF::WriteState(STATE s, Dscalar time, int rec)
    ...
    ArrayHandle<Dscalar> h_tau(s->AdaptationTime,access_location::host,access_mode::read);
...
//    means0 = means0/Nv;
    ...
    AdaptationTimeVar -> put_rec(&tau[0], rec);
...
void SPVDatabaseNetCDF::ReadState(STATE t, int rec,bool geometry)
    ...
    AdaptationTimeVar  ->  get(&taudata[0],1,   NvDim->size());
    ...
    ArrayHandle<Dscalar> h_tau(t->AdaptationTime,access_location::host,access_mode::overwrite);
